...
Page({
  ...
  onLoad: function (options) {
   ...
   wx.request({
     method: 'POST',
     url: getApp().globalData.baseUrl + "data/user/getPicker.do?", //仅为示例，并非真实的接口地址
     data: {
  ...
  submit: function (e) {
    if (this.data._num==1){ //添加硕士数据
      ...
      wx.request({
        method: 'POST',
        url: getApp().globalData.baseUrl + "data/user/insertMasterData.do?", //仅为示例，并非真实的接口地址
        data: {
      ...
      wx.request({
        method: 'POST',
        url: getApp().globalData.baseUrl + "data/user/insertBachelorData.do?", //仅为示例，并非真实的接口地址
        data: {
          userId: wx.getStorageSync("userId"),//用户Id
